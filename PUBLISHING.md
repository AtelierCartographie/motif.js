...
Ce document explique comment publier de nouvelles versions de `@ateliercartographie/motif.js` sur npm.
...
Éditez `CHANGELOG.md` pour documenter vos changements :
...
Ce projet utilise désormais **Trusted Publishing** npm avec OIDC (sans `NPM_TOKEN`).

### Prérequis

- Repository **public**
- Workflow GitHub Actions sur un runner **GitHub-hosted**
- npm CLI **11.5.1+** et Node **22.14.0+** côté workflow

### 1. Configurer le Trusted Publisher sur npmjs.com

1. Allez sur npmjs.com → votre package `@ateliercartographie/motif.js` → **Settings** → **Trusted publishing**
2. Choisissez **GitHub Actions**
3. Renseignez :
   - **Organization or user**: `AtelierCartographie`
   - **Repository**: `motif.js`
   - **Workflow filename**: `publish-npm.yml`
   - **Environment name**: laisser vide (ou votre environnement GitHub si utilisé)
4. Enregistrez
...
- npm : https://www.npmjs.com/package/@ateliercartographie/motif.js
- GitHub Releases : https://github.com/AtelierCartographie/motif.js/releases
...
- Vérifiez la version Node/npm du job CI
